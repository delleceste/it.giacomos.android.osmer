...
public class Grid 
{
	...
	public int width = 0, height = 0; /* default osmer img size */
	...
	public void init(String config, double xc, double yc, int w, int h)
	{
		this.width = w;
		this.height = h;
		this.xc = xc;
		...
		if(this.width > 0 && this.height > 0)
		{
			...
			if(config.length() > 0)
			{
				...
				ystep = this.height / this.nrows; /* height of a single cell */
				...
				for(r = 0; r < this.nrows; r++)
				{
					...
					{
						ele = this.get(r, c);
						ele->xstart = xc - w/2 + c * xstep;
						ele->xend = ele->xstart + xstep;
						ele->ystart = yc - h/2 + r * ystep;
						ele->yend = ele->ystart + ystep;
					}
				}
			}
			else
				echo "<p>Error: file conffile has no contents</p>";
		}
		else	
			echo "<p>Error: image width and height must be set before calling init, with setImgSize</p>";
	}
