...
import it.giacomos.android.osmer.widgets.map.MapBaloonInfoWindowAdapter;
import it.giacomos.android.osmer.widgets.map.OMapFragment;
import it.giacomos.android.osmer.widgets.map.OOverlayInterface;
import it.giacomos.android.osmer.widgets.map.OverlayType;
...
import it.giacomos.android.osmer.widgets.map.report.network.ReportUpdater;
import it.giacomos.android.osmer.widgets.map.report.network.ReportUpdaterListener;
...
//				+ " dataSize " + mDataInterfaceList.size());
	...
	public void update(Context ctx, boolean force, Location location) 
	{
		mMapFrag.getActivity().findViewById(R.id.mapProgressBar).setVisibility(View.VISIBLE);
		mReportUpdater.update(force, location);
	}
